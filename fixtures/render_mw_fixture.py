#!/usr/bin/env python3
"""SXT-035: render the scene-modwheel reference fixtures through the pinned
oracle.

Fixture = factory preset `Basses/Attacky.fxp` (census-blob verified, the
landed SXT-022 voice-slice preset). Its own preset modwheel routes (Filter 1
Cutoff 34.425 st / Filter 1 Resonance 0.383) are preset content and stay
active; PLUS the one modulation route declared in
`model/voice/attacky_mw_inputs.json`, added at runtime through the engine's
host modulation API (`setModDepth01`, ms_modwheel -> scene A `vca_level`,
"A VCA Gain", normalized depth 0.4). The engine render of this configuration
is the oracle reference for the leaf.
...
Policies (fixtures/render_fixture.py conventions):
  ...
  ROUTES   applied after loadPatch, before settle, from the committed
           extractor sidecar (normalized depth; read back and verified,
           including the preset-content routes).
  ...
  DRY      the fixture preset has no FX (all 16 slots Off); the render is
           the dry bus by construction (AGENTS.md comparison rule).
...
def render_bus(surgepy, preset_abs, seq, fixture_routes):
    ...
    try:
        ...
        # dry gate: the fixture preset must carry no FX
        ...
        if any(t != C.fxt_off for t in types):
            raise Refuse(f"fixture preset carries active FX: {types}")

        ms = s.getModSource(C.ms_modwheel)
        applied = []
        for r in fixture_routes:
            if r["dest_param"] == "vca_level":
                target = s.getPatch()["scene"][0]["vca_level"]
            else:
                raise Refuse(f"unexpected fixture route {r['dest_param']}")
            if not s.isValidModulation(target, ms):
                raise Refuse(f"modulation invalid: {r['dest_param']}")
            s.setModDepth01(target, ms, r["normalized_depth"], 0, 0)
            got01 = s.getModDepth01(target, ms, 0, 0)
            if abs(got01 - r["normalized_depth"]) > 1e-6:
                raise Refuse(f"route depth01 readback {got01}")
            applied.append({**r, "depth01_readback": got01})
...
def main():
    ...
    with open(MW_INPUTS, encoding="utf-8") as f:
        mw_in = json.load(f)
    ...
    expected = mw_in["preset"]["census_blob_sha1"]
    ...
    for _ in range(args.repeats):
        mono, info = render_bus(surgepy, preset_abs, seq,
                                mw_in["fixture_routes"])
    ...
    wav_path = os.path.join(args.out_dir,
                            f"reference-{seq['id']}-mw-fixture.wav")
    oc.write_wav_mono16(wav_path, renders[0], SR)

    sidecar = {
        "schema_version": 1,
        "issue": "SXT-035",
        "fixture_id": f"attacky-mw__{seq['id']}",
        "preset": {
            "path": PRESET_REL,
            "census_blob_sha1": expected,
            "actual_blob_sha1": actual,
        },
        "fixture_modification": "one modulation route added at runtime via "
                                "setModDepth01 (host modulation API) after "
                                "loadPatch; preset file untouched; the "
                                "preset's own modwheel routes (Filter 1 "
                                "Cutoff/Resonance) remain active; synthetic "
        ...
        "tool": tool_version(),
        "engine": {"commit": mw_in["engine"]["commit"],
                   "version_string": mw_in["engine"]["version_string"]},
    }
