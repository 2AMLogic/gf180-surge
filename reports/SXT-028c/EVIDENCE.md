...
**Change note (issue #100, 2026-09-25): re-graded under the stronger
wet-path tail gate; all six verdicts unchanged (PASS stays PASS).** The
table above is the original record. It graded the tail on *reference-tail
presence* only, over a hard-coded last-2.0 s window. `compare_chorus_reference.py`
now reads the tail region from each fixture sidecar's declared
`render.frames` / `render.tail_s` (2.5 s; region `[frames − 120000, frames)`),
refuses (NO_VERDICT) when the sidecar does not declare it, and requires the
same legs as the shared comparator (#93) on the mono sum **and** on L and R:
region covered by both renders, reference tail present, model tail present,
and tail residual ≤ −20 dB relative to the reference tail RMS
(`[PROPOSED-TO-BE-FROZEN-AT-PILOT]`). The six committed artifacts were
regenerated by `tools/stereo_tail_gate_checks.py --write-artifacts`
(schema_version 1 → 2). Every changed key is attributed in
`reports/stereo-comparator-tail-gate/artifacts/sxt028c-rerun.txt`: new
gate fields, the removed 2.0 s `channels.tail_mono` block, the reworded
PASS text, `tail_check.tail_rms_rel_db` moving to the declared window,
and last-ULP `spectral_corr` drift. There are 0 unexplained differences
and 0 verdict-status changes:

| Case | declared tail region | tail residual mono / L / R (dB, ≤ −20) | gate | Verdict |
|---|---|---|---|---|
| fmcombo × notes | [153600, 273600) | −73.46 / −73.57 / −73.37 | PASS | **PASS** (PENDING-FREEZE) |
| fmcombo × poly-8 | [57600, 177600) | −83.29 / −82.83 / −83.61 | PASS | **PASS** (PENDING-FREEZE) |
| fmtwang2 × notes | [153600, 273600) | −70.48 / −67.55 / −70.79 | PASS | **PASS** (PENDING-FREEZE) |
| fmtwang2 × poly-8 | [57600, 177600) | −83.94 / −85.81 / −82.78 | PASS | **PASS** (PENDING-FREEZE) |
| alienappears × notes | [153600, 273600) | −53.84 / −54.23 / −54.09 | PASS | **PASS** (PENDING-FREEZE) |
| alienappears × poly-8 | [57600, 177600) | −60.25 / −60.36 / −60.37 | PASS | **PASS** (PENDING-FREEZE) |

On committed renders derived from this leaf's model, a zeroed tail, a
single dropped channel tail, a far-too-fast decay, and a render truncated
...
| NC-B dropped tail (render truncated 1.5 s early) | **fails** the tail-region agreement check (since #100 also the comparator's own declared-region gate: `reports/stereo-comparator-tail-gate/`) |
