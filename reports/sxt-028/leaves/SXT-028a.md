**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028a · raised by SXT-028 (#21)
...
One algorithm per leaf: Airwindows algorithm leaf: Galactic (id 49). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
Ordered by measured SXT-014 ablation delta: max mean block-RMS Δ 10.093 dB (diagnostic only). Provenance: reports/sxt-014/deltas/fmod09/seq-notes-coverage-v1-bypass-slot05-Airwindows.json; reports/sxt-014/deltas/fmod09/seq-poly-8-v1-bypass-slot05-Airwindows.json.

## Scope and pinned sources

- surge-synthesizer/surge@58914e59c608ed4384ba6002e44c3465c58b2e71: libs/airwindows/src/Galactic.h + libs/airwindows/include/airwindows/AirWinBaseClass.h (vendored at the engine commit; algorithm id registry)
...
- SXT-014 pilot carrier exists: resources/data/patches_factory/Tutorials/Formula Modulator/09 Example - Crossfading Oscillators.fxp (factory, blob 76444c0758e7...) -- reuse the committed SXT-012 fixture WAVs + SXT-014 ablation tree for the comparability leg (same engine pin, sequences, policies); no re-render needed for that leg.
- Proposed additional fixture presets (B4-scope carriers, census-blob sha verified at render): resources/data/patches_3rdparty/Altenberg/Guitars/Temple.fxp (contributor, blob 3c8e6d56f405...); resources/data/patches_3rdparty/Altenberg/Leads/Sine Lead.fxp (contributor, blob d4402c44dfbc...); resources/data/patches_3rdparty/Altenberg/Pads/Unity.fxp (contributor, blob c3978e7ce1fb...)
...
- model/effects/aw-49/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/aw-49-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/aw-49/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028a/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028a.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 38 presets; B4-scope candidates (upper bound): 81 (factory 1, contributor 80).
...
- Hashes: reports/sxt-028/leaves/SXT-028a/newly-enabled.json (full path + blob-SHA-1 lists).
