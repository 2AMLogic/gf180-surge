**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028e · raised by SXT-028 (#21)
...
One algorithm per leaf: Distortion effect leaf (multiband + waveshaper drive). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
No SXT-014 ablation carrier for this feature; ordered by B4-scope carrier frequency (304 presets). Essentiality UNVERIFIED -- listening pending (#9).

## Scope and pinned sources

- surge-synthesizer/surge@58914e59c608ed4384ba6002e44c3465c58b2e71: src/common/dsp/effects/DistortionEffect.{h,cpp} (DistortionEffect::process; band/state-variable stages; WS drive tables via waveshapers)
...
- model/effects/type-distortion/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/type-distortion-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/type-distortion/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028e/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028e.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 158 presets; B4-scope candidates (upper bound): 304 (factory 45, contributor 259).
...
- Hashes: reports/sxt-028/leaves/SXT-028e/newly-enabled.json (full path + blob-SHA-1 lists).
