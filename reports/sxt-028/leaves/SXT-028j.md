**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028j · raised by SXT-028 (#21)
...
One routing form per leaf: Routing-form leaf: Global FX slots 3-4 (global3/global4, extended rack half). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
No SXT-014 ablation carrier for this feature; ordered by B4-scope carrier frequency (48 presets). Essentiality UNVERIFIED -- listening pending (#9).
...
- Routing roles exercised by this leaf's B4-scope carriers: global3, global4. Form scope: the routing/bus wiring, scheduling, return/send levels, and per-instance state on these slots for all B4-scope classes; algorithm behavior stays with the per-algorithm leaves.
...
- model/effects/rf-rf-global34/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/rf-rf-global34-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/rf-rf-global34/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028j/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028j.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 1 presets; B4-scope candidates (upper bound): 48 (factory 0, contributor 48).
...
- Hashes: reports/sxt-028/leaves/SXT-028j/newly-enabled.json (full path + blob-SHA-1 lists).
