**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028g · raised by SXT-028 (#21)
...
One algorithm per leaf: Phaser effect leaf (sst-effects Phaser). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
No SXT-014 ablation carrier for this feature; ordered by B4-scope carrier frequency (155 presets). Essentiality UNVERIFIED -- listening pending (#9).

## Scope and pinned sources

- libs/sst/sst-effects@adcac6950292dacc529651093e7ece2d1c8c0d4b/include/sst/effects/Phaser.h (PhaserEffect stages/lfo shift+scale)
...
- Proposed additional fixture presets (B4-scope carriers, census-blob sha verified at render): resources/data/patches_3rdparty/Argitoth/FX/Reson.fxp (contributor, blob ba7d2c45b8ed...); resources/data/patches_3rdparty/Bluelight/Basses/Bass 11.fxp (contributor, blob bebc1a6c8d18...); resources/data/patches_3rdparty/Bluelight/Basses/Bass 17.fxp (contributor, blob 7c8cd4d1060b...)
...
- model/effects/type-phaser/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/type-phaser-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/type-phaser/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028g/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028g.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 97 presets; B4-scope candidates (upper bound): 155 (factory 51, contributor 104).
...
- Hashes: reports/sxt-028/leaves/SXT-028g/newly-enabled.json (full path + blob-SHA-1 lists).
