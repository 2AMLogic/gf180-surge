**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028i · raised by SXT-028 (#21)
...
One routing form per leaf: Routing-form leaf: Scene-A insert FX slots 3-4 (ains3/ains4, extended rack half). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
No SXT-014 ablation carrier for this feature; ordered by B4-scope carrier frequency (55 presets). Essentiality UNVERIFIED -- listening pending (#9).
...
- Routing roles exercised by this leaf's B4-scope carriers: ains3, ains4. Form scope: the routing/bus wiring, scheduling, return/send levels, and per-instance state on these slots for all B4-scope classes; algorithm behavior stays with the per-algorithm leaves.
...
- Proposed additional fixture presets (B4-scope carriers, census-blob sha verified at render): resources/data/patches_3rdparty/Exquis MPE/Basses/Jigsaw.fxp (contributor, blob 676da8126b33...); resources/data/patches_3rdparty/Exquis MPE/Basses/Resurrection.fxp (contributor, blob e2ccab1d7034...); resources/data/patches_3rdparty/Exquis MPE/Keys/Pixel.fxp (contributor, blob 3a9f5294f5c2...)
...
- model/effects/rf-rf-ains34/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/rf-rf-ains34-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/rf-rf-ains34/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028i/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028i.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 4 presets; B4-scope candidates (upper bound): 55 (factory 0, contributor 55).
...
- Hashes: reports/sxt-028/leaves/SXT-028i/newly-enabled.json (full path + blob-SHA-1 lists).
