**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028b · raised by SXT-028 (#21)
...
- SXT-014 pilot carrier exists: resources/data/patches_factory/Basses/Doomsday.fxp (factory, blob a8a3a395d52c...) -- reuse the committed SXT-012 fixture WAVs + SXT-014 ablation tree for the comparability leg (same engine pin, sequences, policies); no re-render needed for that leg.
- Proposed additional fixture presets (B4-scope carriers, census-blob sha verified at render): resources/data/patches_3rdparty/Altenberg/Basses/Piercing.fxp (contributor, blob d02d14ea299b...); resources/data/patches_3rdparty/Altenberg/Leads/AOE.fxp (contributor, blob 427dedd36235...); resources/data/patches_3rdparty/Argitoth/FX/Computer Language 1.fxp (contributor, blob fed86e4fda95...)
...
- model/effects/type-conditioner/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/type-conditioner-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/type-conditioner/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028b/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028b.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- Strict-FX-complete: 175 presets; B4-scope candidates (upper bound): 365 (factory 37, contributor 328).
...
- Hashes: reports/sxt-028/leaves/SXT-028b/newly-enabled.json (full path + blob-SHA-1 lists).
