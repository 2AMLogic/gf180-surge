**Epic:** #3 (effects expansion) · **Plan:** docs/surge-xt-chip-plan-v0.1-2026-09-20.md §6 (SXT-028 row) · planning ID: SXT-028d · raised by SXT-028 (#21)
...
One routing form per leaf: Routing-form leaf: Global FX slot 2 (second concurrent global FX instance). Preserves per-instance state and tails under the shared instance schedule; no monolithic FX port.

## Recovery ordering (diagnostic)
No SXT-014 ablation carrier for this feature; ordered by B4-scope carrier frequency (445 presets). Essentiality UNVERIFIED -- listening pending (#9).

## Scope and pinned sources

- surge-synthesizer/surge@58914e59c608ed4384ba6002e44c3465c58b2e71: src/common/SurgeSynthesizer.cpp -- process() (fxsendout buses, sendToIndex[fxslot_send1..4], per-scene send_level smoothing, fx return_level, fx_bypass gating), loadFx() (per-slot reload), reorderFx(), enqueueFXOff()
- surge-synthesizer/surge@58914e59c608ed4384ba6002e44c3465c58b2e71: src/common/SurgeStorage.h -- fxslot_* slot index constants, n_send_slots, fxb_* bypass states
- surge-synthesizer/surge@58914e59c608ed4384ba6002e44c3465c58b2e71: src/common/dsp/effects/SurgeSSTFXAdapter.h + SurgeEffect.h (instance construction/suspend per slot)
- Parameter mapping: slot-index/role mapping pinned by SurgeStorage.h fxslot_* constants, cross-checked against graphs.jsonl role data (SXT-011); send leaves pin return_level and per-scene send_level semantics from SurgeSynthesizer::process().
- Routing roles exercised by this leaf's B4-scope carriers: global2. Form scope: the routing/bus wiring, scheduling, return/send levels, and per-instance state on these slots for all B4-scope classes; algorithm behavior stays with the per-algorithm leaves.
...
- Proposed additional fixture presets (B4-scope carriers, census-blob sha verified at render): resources/data/patches_3rdparty/Altenberg/Basses/Ancient FM.fxp (contributor, blob 9e796f283f2d...); resources/data/patches_3rdparty/Altenberg/Basses/Piercing.fxp (contributor, blob d02d14ea299b...); resources/data/patches_3rdparty/Altenberg/Basses/Rounded.fxp (contributor, blob 4403d52200f1...)
...
- [ ] Per-instance state: two concurrent instances of this routing form keep independent histories (dual-slot model-level fixture); RTL/model equality must hold per instance.
...
- model/effects/rf-rf-global2/ -- frozen fixed-point model + README (word lengths, state layout, per-instance state ownership) (model/effects/README.md pattern)
- model/effects/fx_inputs/rf-rf-global2-*.json -- fail-closed input extraction (extract_fx_inputs.py pattern; census blob re-verified, graphs cross-check, drift asserted 0)
- rtl/effects/rf-rf-global2/ -- RTL + iverilog testbench incl. the dual-instance and negative-control benches
- reports/SXT-028d/ -- EVIDENCE.md: verification statuses PASS/FAIL/NOT_RUN/BLOCKED, budgets vs achieved, coverage separate from agreement, ext-mem estimate [PENDING-SXT-016]
- tests/test_sxt028d.py -- generator-consistency + exactness harness guards (oracle-dependent tests skip NOT_RUN without oracle)
...
- #18 SXT-025 (done: integration boundary + scheduling for multi-slot wet paths)

## Newly enabled presets

- Strict-FX-complete: 60 presets; B4-scope candidates (upper bound): 445 (factory 2, contributor 443).
...
- Hashes: reports/sxt-028/leaves/SXT-028d/newly-enabled.json (full path + blob-SHA-1 lists).
