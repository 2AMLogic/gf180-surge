#!/usr/bin/env python3
"""SXT-028j negative controls: each control must DEMONSTRABLY FAIL the check
...
NC-B  dropped tail: the declared tail span (TAIL_SPAN_BLOCKS blocks of SILENT
      input while the ring flag stays true) demonstrably carries nonzero
      audio out of the occupant's registers; a render truncated at the end of
      the input must FAIL both the tail-span coverage check and the final
      per-instance checkpoint check.
NC-C  wrong order: the global3/global4 slot-content permutation (reorderFx /
      tools/ablate_fx.py permute pattern) must FAIL the order-sensitive
      equality check -- both model-side and as a LIVE RTL mutant
      (rf_global34_mutants.sv -DNC_SWAP_ORDER) compared against the frozen
      model.
NC-D  shared state: a mutant that pools the two instances' TDF2 histories
      ...
      equality check -- both model-side and as a LIVE RTL mutant
      (rf_global34_mutants.sv -DNC_SHARED_STATE). Run twice: with two
      DIFFERENT occupants and with the SAME occupant in both slots (the
      real-corpus `String Contrabass.fxp` shape, Airwindows in global3 AND
      global4), because identical arithmetic is exactly the case where a
...
NC-E  stale stub: the RTL harness pins the frozen-model revision hash; a
      ...
      invoking tools/compare_rtl_model_rf_global34.py with a mutated pinned
...
import compare_rtl_model_rf_global34 as cmp_rtl  # noqa: E402

OUT = os.path.join(REPO, "reports", "SXT-028j", "negative-controls")
MUTANT = os.path.join(REPO, "rtl", "effects", "rf-rf-global34",
                      "rf_global34_mutants.sv")
WORKDIR = "/tmp/sxt028j_nc"
...
# grossly -- and the engine's real global occupants are not LTI at all. Both
# legs are reported; see reports/SXT-028j/EVIDENCE.md.
...
def run_rack(nblocks, coeffs3, coeffs4, seed=11, amp=0.3, ring_schedule=None,
             occupied3=True, occupied4=True, silent_from=None):
    """Runs the frozen ExtendedGlobalRack for nblocks; returns
    (outs, final_cp)."""
    rack = rm.ExtendedGlobalRack()
    rs = random.Random(seed)
    outs = []
    for b in range(nblocks):
        rack.apply_control(rm.FXB_ALL_FX, 0,
        ...
        glob_in = True if ring_schedule is None else ring_schedule[b]
        ol, orr, _g = rack.process_block(il, ir, glob_in)
        outs.append((ol, orr))
    return outs, rack.checkpoint()
...
def rtl_mutant_leg(name, defect, case_name, detail):
    """Compile rf_global34_mutants.sv with `defect` and require that it FAILS
    ...
    vvp_path = os.path.join(WORKDIR,
                            f"mutant_{defect.lower()}_{case_name}.vvp")
...
def nc_generic_substitute():
    ...
    rack.apply_control(rm.FXB_ALL_FX, 0, True, True, COEFFS_A,
                       True, True, COEFFS_B)
    rack.slot4 = GenericPassthrough(rm.FXSLOT_GLOBAL4)
    rs = random.Random(11)
    generic_outs = []
    for _b in range(4):
        il, ir = rand_block(rs)
        ol, orr, _g = rack.process_block(il, ir, True)
        generic_outs.append((ol, orr))
    generic_cp = rack.checkpoint()

    faithful_outs, faithful_cp = run_rack(4, COEFFS_A, COEFFS_B, seed=11)
...
def nc_dropped_tail():
    ...
    while the ring flag stays true; the occupant's registers keep producing
    real audio there. A render truncated at the end of the input must FAIL."""
    nfull = TAIL_SIGNAL_BLOCKS + TAIL_SPAN_BLOCKS
    full_outs, full_cp = run_rack(nfull, COEFFS_TAIL, COEFFS_B, seed=22,
                                  amp=0.05, silent_from=TAIL_SIGNAL_BLOCKS)
    trunc_outs, trunc_cp = run_rack(TAIL_SIGNAL_BLOCKS, COEFFS_TAIL, COEFFS_B,
                                    seed=22, amp=0.05,
...
def nc_wrong_order():
    """NC-C: global3 -> global4 vs the SAME two slot occupants permuted."""
    # leg 1: saturating pair -- gross divergence
    ab_sat, ab_cp = run_rack(6, COEFFS_HOT, COEFFS_QUIET, seed=33, amp=400.0)
    ba_sat, ba_cp = run_rack(6, COEFFS_QUIET, COEFFS_HOT, seed=33, amp=400.0)
    sat_diffs = [abs(x - y)
    ...
    ab_lin, _ = run_rack(8, COEFFS_A, COEFFS_B, seed=34)
    ba_lin, _ = run_rack(8, COEFFS_B, COEFFS_A, seed=34)
    lin_diffs = [abs(x - y)
                 for (al, ar), (bl, br) in zip(ab_lin, ba_lin)
                 for x, y in zip(al + ar, bl + br)]
    # Recorded bound, NOT a leg: permuting two occupants whose CONTENT is
    # identical is the identity map by construction, so such a permutation is
    # unobservable at the bus. The issue's "two same-class slots swapped"
    # wording therefore has force only when the two same-class slots carry
    # different parameter values -- which is what the legs above use. Stated
    # so the control's strength is not overstated.
    same_ab, _ = run_rack(6, COEFFS_A, COEFFS_A, seed=35)
    same_ba, _ = run_rack(6, COEFFS_A, COEFFS_A, seed=35)
    legs = [
        ...
        rtl_mutant_leg("live RTL order mutant", "NC_SWAP_ORDER",
                       "both-slots-all-fx",
                       "rf_global34_mutants.sv -DNC_SWAP_ORDER evaluates "
                       "global4 before global3; it must FAIL exact equality "
                       "against the frozen model"),
    ]
    ok = all(x["ok"] for x in legs if x["status"] != "NOT_RUN") and \
        legs[0]["ok"] and legs[1]["ok"]
    return {"control": "NC-C wrong order (global3/global4 slot-content "
                       "permutation)",
            "targets": "order-sensitive equality of the extended-rack series "
                       "chain",
            "recorded_bound": {
                "identical_content_permutation_is_an_identity":
                    same_ab == same_ba,
                "note": "permuting two slots whose occupant CONTENT is "
                        "byte-identical cannot change the bus (identity map). "
                        "The issue's 'two same-class slots swapped' control "
                        "therefore has force only when the two same-class "
                        "slots differ in their parameter values; the legs "
                        "above use distinct occupants for exactly that "
                        "reason.",
            },
            "legs": legs,
...
def nc_shared_state():
    """NC-D: a mutant that pools global3's and global4's TDF2 registers into
    ONE instance (tb_fx_shared_line.sv pattern) must FAIL the dual-instance
    ...
    def pooled_run(coeffs3, coeffs4, seed):
        ...
        class MutantRack(rm.ExtendedGlobalRack):
            def __init__(self):
                super().__init__()
                self.slot3 = SharedSlot(shared_bq, rm.FXSLOT_GLOBAL3)
                self.slot4 = SharedSlot(shared_bq, rm.FXSLOT_GLOBAL4)

            def process_block(self, in_l, in_r, glob_in):
                shared_bq.set_coeffs(*coeffs3)
                bus_l, bus_r = list(in_l), list(in_r)
                glob = glob_in
                bus_l, bus_r, glob = self.slot3.process_ringout(bus_l, bus_r,
                                                                glob)
                shared_bq.set_coeffs(*coeffs4)
                bus_l, bus_r, glob = self.slot4.process_ringout(bus_l, bus_r,
                                                                glob)
        ...
        for _b in range(6):
            il, ir = rand_block(rs)
            ol, orr, _g = mrack.process_block(il, ir, True)
            outs.append((ol, orr))
        return outs, mrack.checkpoint()

    mutant_outs, mutant_cp = pooled_run(COEFFS_A, COEFFS_B, 44)
    isolated_outs, isolated_cp = run_rack(6, COEFFS_A, COEFFS_B, seed=44)

    # Same-class variant: the corpus really does host the SAME FX class in
    # both extended-rack slots (John Valentine/Strings/String Contrabass.fxp,
    # Airwindows in global3 AND global4). Identical arithmetic in both slots
    # is exactly where a pooled-state implementation is most likely to slip
    # through, so the control is repeated there.
    same_mutant_outs, same_mutant_cp = pooled_run(COEFFS_A, COEFFS_A, 45)
    same_iso_outs, same_iso_cp = run_rack(6, COEFFS_A, COEFFS_A, seed=45)
    ...
    legs = [
        ...
        leg("model pooled-history mutant (SAME-class occupants)",
            ...
            "the real-corpus same-class shape (String Contrabass.fxp: "
            "Airwindows in both global3 and global4) must ALSO catch pooled "
            "state -- identical arithmetic is where pooling hides best",
            {"blocks_compared": 6,
        ...
        rtl_mutant_leg("live RTL shared-state mutant", "NC_SHARED_STATE",
                       "both-slots-all-fx",
                       "rf_global34_mutants.sv -DNC_SHARED_STATE pools both "
        ...
        rtl_mutant_leg("live RTL shared-state mutant (SAME-class case)",
                       ...
                       "the same-class dual-occupant case, where both slots "
                       "run identical coefficients"),
    ]
    ok = all(x["ok"] for x in legs if x["status"] != "NOT_RUN") and \
        legs[0]["ok"] and legs[1]["ok"] and legs[2]["ok"]
    return {"control": "NC-D shared state (both extended-rack slots pooled "
                       "into one history)",
...
def nc_stale_stub():
    ...
    proc = subprocess.run(
        [sys.executable,
         os.path.join(REPO, "tools", "compare_rtl_model_rf_global34.py"),
...
def main():
    ...
    out_doc = {"schema_version": 1, "leaf": "SXT-028j",
