#!/usr/bin/env python3
"""SXT-040 reference renderer (requires the external pinned oracle).

Renders the fixture sequences through the pinned engine for the DECLARED
Sine-family fixture configurations (from inputs/<carrier>.json): the
...
def main():
    ...
    with open(seq_path, encoding="utf-8") as f:
        seq = json.load(f)
    inputs_rel = os.path.join(REPO, "model", "oscillators", "sine",
    ...
    sidecar = {
        ...
        "isolation_note": "declared fixture configuration (one Sine slot "
                          "audible; other mixer paths, filter units, FX, "
                          "waveshaper, scene lowcut, FM routing off; fbc "
                          "serial1; scene mode Single; retrigger on) - a "
                          "test configuration, never an adapted preset, "
                          "never coverage",
