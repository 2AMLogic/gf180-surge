// SXT-038 NEGATIVE CONTROL (RTL mutant) — DO NOT USE AS THE LEAF RTL.
//
// This is rtl/voice/tb_lp24.sv with exactly ONE constant mutated: the
// round-half-up bias inside qmul() is 2^19 instead of 2^21/2 = 2^20.  The
// exactness harness (tools/compare_rtl_model_lp24.py) MUST report FAIL on
// this file; if it ever reports PASS, the exactness check is not actually
// checking the arithmetic and the leaf's claim (1) is void.
//
...
module tb_lp24;   // MUTANT BUILD
  ...
  function automatic signed [31:0] qmul(input signed [31:0] a, input signed [31:0] b);
    ...
    qmul_count++;
    p = a * b;
    r = (p + (64'sd1 << 19)) >>> 21;   // MUTATION: rounding bias 2^19 (was 2^20)
