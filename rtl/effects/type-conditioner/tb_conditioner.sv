...
module tb_conditioner;
    ...
    task automatic env_step(input integer inst, input signed [63:0] la);
        begin
            flam[inst] = add_c(mul_cc(sub_c(ONE_C, cw[inst][0]), flam[inst]),
                               mul_cc(cw[inst][0], la));
            flam2[inst] = add_c(mul_cc(sub_c(ONE_C, cw[inst][1]), flam2[inst]),
                                mul_cc(cw[inst][1], flam[inst]));
            if (flam[inst] > flam2[inst]) flam2[inst] = flam[inst];
            gain_r[inst] = recip_c(flam2[inst]);
        end
    endtask
    ...
    task automatic clear_limiter(input integer inst);
        ...
        begin
            for (t = 0; t < 128; t = t + 1) begin
                dly_l[inst][t] = 0; dly_r[inst][t] = 0; lamax[inst][t] = 0;
            end
            bufpos[inst] = 0;
            flam[inst] = ONE_C; flam2[inst] = ONE_C; gain_r[inst] = ONE_C;
            rhash[inst] = 0;
        end
    endtask
    ...
    task automatic process_block(input integer inst);
        ...
        begin
            ...
            for (k = 0; k < 32; k = k + 1) begin
                ...
                rhash[inst] = rhash[inst] + (x64(lsq) - x64(lamax[inst][bp])) * (bp + 257);
                lamax[inst][bp] = lsq;

                out_l[k] = mul_ca(gain_r[inst], d_l);
                out_r[k] = mul_ca(gain_r[inst], d_r);
                bufpos[inst] = (bp + 1) & LOOKAHEAD_MASK;
            end
            for (k = 0; k < 32; k = k + 1) begin
    ...
    task automatic emit_state(input integer bb, input integer inst);
        ...
        begin
            $fwrite(fd, "T %0d %0d bufpos %0d flamax %0d flamax2 %0d gain %0d",
                    bb, inst, bufpos[inst], flam[inst], flam2[inst], gain_r[inst]);
