// SXT-028h Scene-B insert FX bus, slots 1-2 (routing form) iverilog harness.
//
// Instantiates rf_bins12_core, drives it from a per-block control-plane
// stream (rf_bins12_core's config bus, poked hierarchically -- the
...
// (tools/compare_rtl_model_rf_bins12.py).
//
// This single harness carries BOTH required benches:
//   * the dual-instance bench -- every checkpoint line dumps slot1's and
//     slot2's TDF2 registers separately, so a pooled-history implementation
...
//     order, truncated tail spans, a stale frozen-revision pin and a
//     mid-tail reload through exactly this stimulus path.
//
// Stimulus (emitted by tools/compare_rtl_model_rf_bins12.py):
//   +CTRLFILE=<path>   17 words/block: fx_bypass, fx_disable,
//                      slot1_occupied, slot1_reload, slot2_occupied,
//                      slot2_reload, sc_in, slot1_c[0..4], slot2_c[0..4]
//                      (Q3.29)
...
module tb_rf_bins12;
  ...
  initial begin
    ...
    if (!$value$plusargs("TRACE=%s", trace_name)) trace_name = "/tmp/rf_bins12_trace.txt";
    ...
    for (b = 0; b < nblocks; b++) begin
      ...
      core.cfg_slot1_reload   = ctrl_mem[b*CTRL_WORDS + 3][0];
      core.cfg_slot2_occupied = ctrl_mem[b*CTRL_WORDS + 4][0];
      core.cfg_slot2_reload   = ctrl_mem[b*CTRL_WORDS + 5][0];
      core.sc_in              = ctrl_mem[b*CTRL_WORDS + 6][0];
      for (i = 0; i < 5; i++) begin
        core.cfg_slot1_c[i] = $signed(ctrl_mem[b*CTRL_WORDS + 7 + i]);
        core.cfg_slot2_c[i] = $signed(ctrl_mem[b*CTRL_WORDS + 12 + i]);
      end
  ...
  // Dual-instance checkpoint: slot1's and slot2's registers are dumped as
