// SXT-028i NEGATIVE-CONTROL RTL ONLY -- deliberately defective variants of
// rf_ains34_core, compiled INSTEAD of rtl/effects/rf-rf-ains34/
// rf_ains34_core.sv by tools/rf_ains34_negative_controls.py and required to
// FAIL the exact-equality check against the frozen model
// (model/effects/rf-rf-ains34/rf_ains34_model.py). A control that PASSES is
// a broken control (a finding), not a success.
//
// This file declares the SAME module name (`rf_ains34_core`) on purpose so
// that the production testbench (tb_rf_ains34.sv) and the production
...
//   iverilog -g2012 -DNC_SWAP_ORDER ...     the ains3 -> ains4 series chain
//       is evaluated in the WRONG order (ains4 first; the reorderFx /
//       tools/ablate_fx.py permute defect) -- must fail the order-sensitive
//       equality.
//   iverilog -g2012 -DNC_TAIL_KILL ...      the stage is skipped whenever the
//       incoming block is all-zero (the plausible "nothing to do on silence"
//       optimisation that DROPS the occupant's arithmetic tail and freezes
//       its state) -- must fail the tail case's equality while remaining
//       UNDETECTED on a non-silent baseline case, which is precisely why the
//       declared tail span is a load-bearing acceptance case rather than a
//       formality.
//
...
module rf_ains34_core (
  ...
  logic       sc_cur;
`ifdef NC_TAIL_KILL
  // DEFECT (NC_TAIL_KILL) support: "is the incoming block entirely silent?"
  logic       blk_silent;
`endif
...
`ifdef NC_TAIL_KILL
          ...
          for (int i = 0; i < BLOCK; i++)
            if (in_l[i] != 32'sd0 || in_r[i] != 32'sd0) blk_silent <= 1'b0;
`endif
...
`ifdef NC_SWAP_ORDER
        // DEFECT (NC_SWAP_ORDER): the series chain is evaluated ains4 FIRST,
        // then ains3 -- the slot-content permutation the wrong-order control
...
`elsif NC_TAIL_KILL
        // DEFECT (NC_TAIL_KILL): both stages are skipped when the incoming
        // block is silent, so the occupant's arithmetic tail is dropped and
        // its registers freeze instead of decaying.
        S_S3: begin
          if (run_s3 && sc_cur && !blk_silent) biquad_step(0, k);
          if (k == BLOCK-1) begin
            k  <= 6'd0;
            st <= S_S4;
          end else k <= k + 1'b1;
        ...
        S_S4: begin
          if (run_s4 && sc_cur && !blk_silent) biquad_step(1, k);
          if (k == BLOCK-1) begin
            sc_out <= sc_cur;
            st     <= S_DONE;
          end else k <= k + 1'b1;
...
`ifdef NC_SHARED_STATE
    ...
    op0  = (144'(out_l[kk]) * c0) + s3_reg0[0];
    nr00 = (144'(out_l[kk]) * c1) + s3_reg1[0]
    ...
    op1  = (144'(out_r[kk]) * c0) + s3_reg0[1];
    nr01 = (144'(out_r[kk]) * c1) + s3_reg1[1]
...
`ifndef NC_TAIL_KILL
  initial $fatal(1, "rf_ains34_mutants.sv compiled with no defect selected: use -DNC_SHARED_STATE, -DNC_SWAP_ORDER or -DNC_TAIL_KILL");
`endif
`endif
`endif
