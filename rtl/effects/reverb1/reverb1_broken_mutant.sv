// SXT-024 NEGATIVE CONTROL (broken mutant; NOT a product path).
// Injected defect: the tap-7 buffer WRITE is silently skipped, leaving a
// stale composite-tap line in external memory. tools/run_reverb_rtl.py must
// FAIL this mutant on the exactness comparison (transaction log, output
// words, and external-memory image all diverge) -- the SXT-022
// injected-defect standard adapted to long-buffer state. Module name is kept
// as reverb1_core so it is a drop-in replacement for the harness.
  ...
  always @* begin
    ...
    if (running) begin
      case (st)
        ...
        S_TAP_WR: begin
          if (t != 5'd7) begin            // DEFECT: tap-7 write silently skipped
            em_req   = 1'b1;
            em_we    = 1'b1;
