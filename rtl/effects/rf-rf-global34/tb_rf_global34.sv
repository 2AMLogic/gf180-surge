// SXT-028j Global FX slots 3-4 (routing form, extended rack half) iverilog
// harness.
//
// Instantiates rf_global34_core, drives it from a per-block control-plane
// stream (rf_global34_core's config bus, poked hierarchically -- the
...
// (tools/compare_rtl_model_rf_global34.py).
//
// This single harness carries BOTH required benches:
//   * the dual-instance bench -- every checkpoint line dumps slot3's and
//     slot4's TDF2 registers separately, so a pooled-history implementation
...
//
// Stimulus (emitted by tools/compare_rtl_model_rf_global34.py):
//   +CTRLFILE=<path>   17 words/block: fx_bypass, fx_disable,
//                      slot3_occupied, slot3_reload, slot4_occupied,
//                      slot4_reload, glob_in, slot3_c[0..4], slot4_c[0..4]
//                      (Q3.29)
//   +INFILE=<path>     64 words/block: 32 x in_l, 32 x in_r (Q10.21) -- the
//                      master bus as the upstream global1->global2 segment
//                      left it
//   +NBLOCKS=n
...
module tb_rf_global34;
  ...
  initial begin
    ...
    if (!$value$plusargs("TRACE=%s", trace_name)) trace_name = "/tmp/rf_global34_trace.txt";
    ...
    for (b = 0; b < nblocks; b++) begin
      ...
      core.glob_in            = ctrl_mem[b*CTRL_WORDS + 6][0];
      for (i = 0; i < 5; i++) begin
  ...
  // Dual-instance checkpoint: slot3's and slot4's registers are dumped as
