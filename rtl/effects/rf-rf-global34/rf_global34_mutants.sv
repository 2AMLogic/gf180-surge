// SXT-028j NEGATIVE-CONTROL RTL ONLY -- deliberately defective variants of
// rf_global34_core, compiled INSTEAD of rtl/effects/rf-rf-global34/
// rf_global34_core.sv by tools/rf_global34_negative_controls.py and required
// to FAIL the exact-equality check against the frozen model
// (model/effects/rf-rf-global34/rf_global34_model.py). A control that PASSES
// is a broken control (a finding), not a success.
//
// This file declares the SAME module name (`rf_global34_core`) on purpose so
// that the production testbench (tb_rf_global34.sv) and the production
...
//   iverilog -g2012 -DNC_SHARED_STATE ...   both global slots POOL one
...
//   iverilog -g2012 -DNC_SWAP_ORDER ...     the global3 -> global4 series
//       chain is evaluated in the WRONG order (global4 first; the reorderFx
//       / tools/ablate_fx.py permute defect) -- must fail the
...
`ifdef NC_SWAP_ORDER
        // DEFECT (NC_SWAP_ORDER): the series chain is evaluated global4
        // FIRST, then global3 -- the slot-content permutation the
        // wrong-order control targets. Everything else is identical to the
        // real core.
        S_S3: begin
...
`ifndef NC_SWAP_ORDER
  initial $fatal(1, "rf_global34_mutants.sv compiled with no defect selected: use -DNC_SHARED_STATE or -DNC_SWAP_ORDER");
`endif
`endif
