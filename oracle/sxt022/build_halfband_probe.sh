#!/usr/bin/env bash
# SXT-022 (#123) — build the HalfRateFilter::process_block_D2 branch-assignment
# probe against the manifest-pinned sst-filters submodule.
#
# Checks out the manifest-pinned GPL filter submodule into an EXTERNAL
# directory (never inside this repository) and compiles
# oracle/sxt022/halfband_d2_probe.cpp against it.  The resulting binary is a
# GPL-3.0-or-later combined work; it stays in that external directory and is
# never committed or distributed (decision-records/0009, 0010).
#
# Pins are read from oracle/manifest.json; any drift aborts (fail-closed).
#
# Usage:
#   ./oracle/sxt022/build_halfband_probe.sh [--dir <external-dir>]
# Environment:
#   SXT022_ORACLE_DIR   external build root (default ~/.cache/sxt022-oracle)
# Prints the probe binary path on stdout.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
DIR="${SXT022_ORACLE_DIR:-$HOME/.cache/sxt022-oracle}"
while [ $# -gt 0 ]; do
  case "$1" in
    --dir) DIR="$2"; shift 2 ;;
...
OUT="$DIR/halfband_d2_probe"
# -ffp-contract=off: no FMA contraction, so the reference arithmetic is the
# plain IEEE single-precision sequence the pinned kernel spells out.
# -DSIMDE_UNAVAILABLE: use the host's native SSE intrinsics instead of simde;
# process_block_D2 uses only shuffle/movehl/movelh/add/sub/mul, which are
# IEEE-identical on both paths.
g++ -O2 -std=c++20 -msse4.2 -ffp-contract=off -DSIMDE_UNAVAILABLE \
    -I "$DIR/sst-filters/include" -I "$DIR/sst-basic-blocks/include" \
    "$REPO/oracle/sxt022/halfband_d2_probe.cpp" -o "$OUT"

{
  echo "probe_source_sha256 $(sha256sum "$REPO/oracle/sxt022/halfband_d2_probe.cpp" | cut -d' ' -f1)"
